#handle_entities_ocs
handle_entities_ocs <- function(handler, source, config, validate = TRUE, handle = TRUE){
  ...
  entities <- switch(mime::guess_type(entities_file),
                     "text/csv" = {
                       ...
                       handle_entities_csv(handler = handler, source = entities_file, config = config, validate = validate, handle = handle)
                      },
                     "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" = {
                       handle_entities_excel <- source(system.file("metadata/entity", "entity_handler_excel.R", package = "geoflow"))$value
                       handle_entities_excel(handler = handler, source = entities_file, config = config, validate = validate, handle = handle)
                     }
  )
  return(entities)
}