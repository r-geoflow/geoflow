    ...
    if(length(entity$rights)>0){
        ...
        zenodo_metadata$setAccessRight(accessRight)
          if ("embargoed" %in% accessRight) {
            ...
            embargoDate <- if(length(embargoDates)>0) as.Date(embargoDates[[1]]$values[[1]]) else config$logger.error(sprintf("Zenodo: 'embargo' not set in entity$dates whereas 'embargoed' is set in entity$rights. Please check your entities !"))
