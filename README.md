...
* ``actions``: one ore more actions to execute in the workflow. In the case of an entity-based workflow, the user can add either embedded actions made available in geoflow (The list can be obtained with ``list_actions()``) or custom actions the user wants to configure.
